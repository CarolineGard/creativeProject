...
function createTrainGeometry() {
    const nose    = new THREE.CylinderBufferGeometry( 0.75, 0.75, 3.25, 12);
...
export function createTrainMesh() {
    // Create group to hold the meshes
    const train = new THREE.Group();
    scene.add(train);

    const materials = trainMaterials();
    const geometries = createTrainGeometry();
    ...
    smallWheelRear.position.set(0, -0.25, 0);
